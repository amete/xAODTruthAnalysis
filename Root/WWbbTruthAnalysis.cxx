...
ClassImp(WWbbTruthAnalysis)

//using namespace RestFrames;

WWbbTruthAnalysis :: WWbbTruthAnalysis ()
{
...
EL::StatusCode WWbbTruthAnalysis :: setupJob (EL::Job& job)
{
...
EL::StatusCode WWbbTruthAnalysis :: histInitialize ()
{
  ...
  if(saveTree) {
    ...
    //WWbb
    outputTree->Branch("mHH"           /* n/a             */, &m_br_mHH              );
    outputTree->Branch("mHH_bC"        /* n/a             */, &m_br_mHH_bC           );
    outputTree->Branch("pThh"          /* n/a             */, &m_br_hh_pt            );
    outputTree->Branch("pThh_bC"       /* n/a             */, &m_br_hh_pt_bC         );
    outputTree->Branch("dphi_hh"       /* n/a             */, &m_br_dphi_hh          );
    outputTree->Branch("dphi_hh_bC"    /* n/a             */, &m_br_dphi_hh_bC       );
    outputTree->Branch("h_pt"          /* n/a             */, &m_br_h_pt             );
    outputTree->Branch("h_pt_bC"       /* n/a             */, &m_br_h_pt_bC          );
    outputTree->Branch("dRll"          /* n/a             */, &m_br_dRll             );
    outputTree->Branch("HT2Ratio"      /* n/a             */, &m_br_HT2Ratio         );
    outputTree->Branch("HT2Ratio_bC"   /* n/a             */, &m_br_HT2Ratio_bC      );
    outputTree->Branch("mt2_bb"        /* n/a             */, &m_br_mt2_bb           );
    outputTree->Branch("mt2_bb_bC"     /* n/a             */, &m_br_mt2_bb_bC        );
    outputTree->Branch("mt2_llbb"      /* n/a             */, &m_br_mt2_llbb         );
    outputTree->Branch("mt2_llbb_bC"   /* n/a             */, &m_br_mt2_llbb_bC      );
    outputTree->Branch("mbb"           /* n/a             */, &m_br_mbb              );
    outputTree->Branch("mbb_bC"        /* n/a             */, &m_br_mbb_bC           );
    outputTree->Branch("MT_1"          /* n/a             */, &m_br_MT_1             );
    outputTree->Branch("MT_1_bC"       /* n/a             */, &m_br_MT_1_bC          );
...
EL::StatusCode WWbbTruthAnalysis :: fileExecute ()
{
...
EL::StatusCode WWbbTruthAnalysis :: changeInput (bool /*firstFile*/)
{
...
EL::StatusCode WWbbTruthAnalysis :: initialize ()
{
...
EL::StatusCode WWbbTruthAnalysis :: execute ()
{
  ...
  if(saveTree) {
    ...
    m_br_dRll = -1.0;
    m_br_HT2Ratio = -1.0;
    m_br_HT2Ratio_bC = -1.0;
    m_br_mt2_bb = -1.0;
    m_br_mt2_bb_bC = -1.0;
    m_br_mt2_llbb = -1.0;
    m_br_mt2_llbb_bC = -1.0;
    m_br_mbb = -1.0;
    m_br_mbb_bC = -1.0;
    m_br_MT_1 = -1.0;
    m_br_MT_1_bC = -1.0;
...
////  // To check filter!!!!!
  ...
  double dRll = lep0_tlv.DeltaR(lep1_tlv);
...
if(saveHists) {
  ...
  if(saveTree) {
    ...
    std::vector<TLorentzVector> bjets_bC;
    ...
    if(bjets.size()>=2) {
        ...
        // HT2Ratio
        float numerator = (met_tlv + lep0_tlv + lep1_tlv).Pt() * MEVtoGEV;
        numerator += (bjets.at(0) + bjets.at(1)).Pt() * MEVtoGEV;
        float denominator = met_tlv.Pt() * MEVtoGEV;
        denominator += lep0_tlv.Pt() * MEVtoGEV;
        denominator += lep1_tlv.Pt() * MEVtoGEV;
        denominator += bjets.at(0).Pt() * MEVtoGEV;
        denominator += bjets.at(1).Pt() * MEVtoGEV;
        m_br_HT2Ratio = (numerator / denominator);

        // mt2_bb
        ComputeMT2 calc_mt2_bb = ComputeMT2(bjets.at(0), bjets.at(1), met_tlv, 0., 0.);
        m_br_mt2_bb = calc_mt2_bb.Compute() * MEVtoGEV;

        // mt2_llbb
        ComputeMT2 calc_mt2_llbb = ComputeMT2( lep0_tlv + lep1_tlv, bjets.at(0) + bjets.at(1), met_tlv, 0., 0.);
        m_br_mt2_llbb = calc_mt2_llbb.Compute() * MEVtoGEV;

        // mbb
        m_br_mbb = (bjets.at(0) + bjets.at(1)).M() * MEVtoGEV;

        // MT_1
        TLorentzVector vis = (lep0_tlv + lep1_tlv + bjets.at(0) + bjets.at(1));
        float pt_vis = vis.Pt();
        float m_vis = vis.M();
        float et_vis = sqrt(pt_vis * pt_vis + m_vis * m_vis);
        m_br_MT_1 = sqrt( (et_vis + met_tlv.Pt()) * (et_vis + met_tlv.Pt()) -
                            ( (vis + met_tlv).Pt() * (vis + met_tlv).Pt()) ) * MEVtoGEV;
    }
    if(bjets_bC.size()>=2) {
        float numerator = (met_tlv + lep0_tlv + lep1_tlv).Pt() * MEVtoGEV;
        numerator += (bjets_bC.at(0) + bjets_bC.at(1)).Pt() * MEVtoGEV;
        float denominator = met_tlv.Pt() * MEVtoGEV;
        denominator += lep0_tlv.Pt() * MEVtoGEV;
        denominator += lep1_tlv.Pt() * MEVtoGEV;
        denominator += bjets_bC.at(0).Pt() * MEVtoGEV;
        denominator += bjets_bC.at(1).Pt() * MEVtoGEV;
        m_br_HT2Ratio_bC = (numerator / denominator);

        // mt2_bb
        ComputeMT2 calc_mt2_bb = ComputeMT2(bjets_bC.at(0), bjets_bC.at(1), met_tlv, 0., 0.);
        m_br_mt2_bb_bC = calc_mt2_bb.Compute() * MEVtoGEV;

        // mt2_llbb
        ComputeMT2 calc_mt2_llbb = ComputeMT2( lep0_tlv + lep1_tlv, bjets_bC.at(0) + bjets_bC.at(1), met_tlv, 0., 0.);
        m_br_mt2_llbb_bC = calc_mt2_llbb.Compute() * MEVtoGEV;

        // mbb
        m_br_mbb_bC = (bjets_bC.at(0) + bjets_bC.at(1)).M() * MEVtoGEV;

        // MT_1
        TLorentzVector vis = (lep0_tlv + lep1_tlv + bjets_bC.at(0) + bjets_bC.at(1));
        float pt_vis = vis.Pt();
        float m_vis = vis.M();
        float et_vis = sqrt(pt_vis * pt_vis + m_vis * m_vis);
        m_br_MT_1_bC = sqrt( (et_vis + met_tlv.Pt()) * (et_vis + met_tlv.Pt()) -
                            ( (vis + met_tlv).Pt() * (vis + met_tlv).Pt()) ) * MEVtoGEV;
...
EL::StatusCode WWbbTruthAnalysis :: postExecute ()
{
...
EL::StatusCode WWbbTruthAnalysis :: finalize ()
{
...
EL::StatusCode WWbbTruthAnalysis :: histFinalize ()
{
...
// Is BJet
bool WWbbTruthAnalysis :: isBJet(const double eta, const int label) {
...
// Print info
void WWbbTruthAnalysis :: printTruthInfo(const xAOD::TruthParticle* particle)
{
